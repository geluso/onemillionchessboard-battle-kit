...
function moveChessPieces() {
  ...
  const myPieces = Array.from(allButtons).filter(button => {
    if (column.includes(button.getAttribute('data-id'))) {
  ...
  console.log(`Found ${myPawns.length} ${isBlack ? 'black' : 'white'} pawns`);
  ...
  function processPiece() {
    ...
    setTimeout(() => {
      ...
      if (moveButtons.length > 0) {
        ...
        console.log(`Clicking destination at y:${destY} (${isBlack ? 'down' : 'up'})`);
        destButton.click();
      } else {
